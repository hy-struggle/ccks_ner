...
def train(model, data_iterator, optimizer, params):
    ...
    for step, _ in enumerate(t):
        ...
        loss = model(input_ids, attention_mask=input_mask, labels=labels, input_word_ids=word_ids,
...
def train_and_evaluate(model, params, restore_file=None):
    ...
    param_pre = [(n, p) for n, p in param_optimizer if 'bert' in n or 'word_embedding' in n]
    # middle model param
    param_middle = [(n, p) for n, p in param_optimizer if
                    not any([s in n for s in ('bert', 'crf', 'word_embedding')]) or 'dym_weight' in n]
    # crf param
