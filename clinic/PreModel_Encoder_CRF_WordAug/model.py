...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config, params):
        ...
        self.custom_wordvec = params.custom_wordvec
        self.word_vec_dir = params.word_vec_dir
        ...
        # word aug
        self.word_embedding = nn.Embedding(num_embeddings=params.word_vocab_size,
                                           embedding_dim=params.word_embedding_dim, padding_idx=0)
        self.word_dim_to_char = nn.Linear(params.word_embedding_dim, config.hidden_size)
    ...
    def reset_params(self):
        # 初始化预训练word embedding
        if not self.custom_wordvec:
            vec_array = []
            with open(self.word_vec_dir, "r", encoding="utf-8") as fin:
                for i, line in enumerate(fin):
                    vec = list(map(float, line.split(' ')[1:]))
                    vec_array.append(vec)
            self.word_embedding.weight.data.copy_(torch.tensor(vec_array))
    ...
    def forward(
        ...
        Args:
            ...
            input_word_ids: (bs, max_word_len)
            word_position_matrix: (bs, max_len, max_word_len)
        ...
        # word features
        # (bs, max_word_len, word_dim) --> (bs, max_word_len, h)
        word_hidden_states = self.word_dim_to_char(self.word_embedding(input_word_ids))
        # (bs, max_len, max_word_len) * (bs, max_word_len, h) = (bs, max_len, h)
        word_hidden_states = torch.bmm(word_position_matrix.float(), word_hidden_states.float())
        # fusion
        sequence_output = sequence_output + word_hidden_states
