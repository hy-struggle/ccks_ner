...
class NERDataLoader(object):
    ...
    def collate_fn(features):
        ...
        bio_labels = torch.tensor([f.tag for f in features], dtype=torch.long)
        cls_labels = torch.tensor([f.cls_tag for f in features], dtype=torch.long)
        ...
        tensors = [input_ids, input_mask, bio_labels, cls_labels, split_to_ori, example_ids]
        return tensors
...
if __name__ == '__main__':
    ...
    feats = datalodaer.get_features(data_sign='test')
    print(feats[212].split_to_original_id)
    print(feats[212].input_mask)
    # print(len(feats))
