...
parser.add_argument('--epoch_num', required=True, type=int, help="指定epoch_num")
...
def train(model, data_iterator, optimizer, params):
    ...
    for step, _ in enumerate(t):
        ...
        input_ids, input_mask, labels, cls_labels, _, _ = batch

        # compute model output and loss
        loss = model(input_ids, attention_mask=input_mask, bio_labels=labels, cls_labels=cls_labels)
...
def train_and_evaluate(model, params, restore_file=None):
    ...
    param_downstream = [(n, p) for n, p in param_optimizer if 'bert' not in n and 'crf' not in n]
    # crf param
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in param_downstream if not any(nd in n for nd in no_decay)],
         'weight_decay': params.weight_decay_rate, 'lr': params.fin_tuning_lr * 10
         },
        # 不衰减
        {'params': [p for n, p in param_downstream if any(nd in n for nd in no_decay)],
         'weight_decay': 0.0, 'lr': params.fin_tuning_lr * 10
         },
        # crf,单独设置学习率
        {'params': param_crf,
