...
def convert_examples_to_features(params, examples, tokenizer, pad_sign=True, greed_split=True):
    ...
    """
    # tag to id
    tag2idx = {tag: idx for idx, tag in enumerate(params.bio_tags)}
    typetag2idx = {tag: idx for idx, tag in enumerate(params.type_tags)}
    features = []
    ...
    for (example_idx, example) in enumerate(examples):
        ...
        for text, start in zip(sub_texts, starts):
            ...
            text_tokens = [tokenizer.tokenize(token)[0] if len(tokenizer.tokenize(token)) == 1 else '[UNK]'
                           for token in text]
            # bio label id
            tag_ids = [tag2idx[tag.split('-')[0]] for tag in example.tag[start:start + len(text)]]
            # cls label id
            cls_ids = [typetag2idx.get(tag.split('-')[-1], -1) for tag in example.tag[start:start + len(text)]]
            # 保存子文本对应原文本的位置
